...
class CrealityCloudUploader(BaseUploader):
    async def upload_model(
        ...
        try:
            await on_progress("Navigiere zu Creality Cloud...", 5)
            await page.goto("https://www.crealitycloud.com/model/upload", wait_until="domcontentloaded", timeout=30_000)
            ...
            logger.error(f"Creality Cloud Upload-Fehler: {e}")
            try:
                await page.screenshot(path="storage/debug_crealitycloud.png")
            except Exception:
                pass
            await page.close()
            raise
